    ...
    <div class="content" v-if="getShoppingCart.length>0">
      <ul>
        ...
        <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.id">
          ...
          <div class="pro-price">{{item.price}}元/每月</div>
          <div class="pro-num">
            <el-input-number
              size="small"
              :value="item.num"
    ...
    methods: {
        ...
        checkChange(val, key) {
         ...
         handleChange(currentValue, key, productID) {
            ...
            // 向后端发起更新购物车的数据库信息请求
            this.$axios
            .post("http://127.0.0.1:7001/default/shopping/updateShoppingCart", {
        ...
        deleteItem(e, id, productID) {
            this.$axios
                .post("http://127.0.0.1:7001/default/shopping/deleteShoppingCart", {
                user_id: this.$store.getters.getUser.user_id,
                product_id: productID
                })
                .then(res => {
